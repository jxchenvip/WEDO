...
gulp.task('wat', function() {
    ...
    gulp.watch([option.postcssPath + '/**/*.css'], [mod]).on('change', reload)
});
...
// 编译
gulp.task('build', ['copy', 'sea'], function() {});
...
gulp.task('createInfo', function() {
    ...
    var pfm = path.split(path.sep)[path.split(path.sep).length - 1];
